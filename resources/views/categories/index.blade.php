    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col justify-between hover:shadow-md transition">
            <div>
                ...
                <p class="text-xs text-slate-600 dark:text-slate-400 font-medium mt-2 min-h-[32px] line-clamp-2">{{ $cat->description ?? 'No description' }}</p>

                <div class="mt-3 text-xs text-slate-600 dark:text-slate-400 font-medium">
                    Logged tasks: <strong class="text-slate-900 dark:text-white font-bold">{{ $cat->work_entries_count }}</strong>
                </div>
            </div>

            <!-- Footer Actions -->
            <div class="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <a href="{{ route('work-entries.index', ['category_id' => $cat->id]) }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-500">
                    View Logs →
                </a>

                <div class="flex items-center gap-1">
                    <button type="button" @click="openEdit(@js($cat))" class="p-1.5 rounded-lg text-slate-500 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">
    ...
    <div x-show="addModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs" style="display: none;">
        <div @click.outside="addModalOpen = false" class="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
            ...
            <form method="POST" action="{{ route('categories.store') }}" class="space-y-4 text-xs">
                @csrf
                <div>
                    <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Category Name *</label>
                    <input type="text" name="name" required placeholder="e.g. 3D Modeling" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-medium text-slate-900 dark:text-white">
                </div>

                <div>
                    <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Color Tag *</label>
                    <div class="flex items-center gap-3">
                        <input type="color" name="color" value="#4F46E5" class="w-10 h-10 rounded-xl border border-slate-200 cursor-pointer p-0.5">
                        <span class="text-xs text-slate-600 dark:text-slate-400 font-medium">Pick color badge representation</span>
                    </div>
                </div>

                <div>
                    <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Description</label>
                    <textarea name="description" rows="2" placeholder="Brief description..." class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white"></textarea>
                </div>
                ...
                <div class="pt-3 flex items-center justify-end gap-2">
                    ...
                    <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-md cursor-pointer">Create Category</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form method="POST" :action="editData.url" class="space-y-4 text-xs">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-xs text-slate-600 dark:text-slate-400 font-medium">Color swatch</span>
                    </div>
                </div>

                <div>
                    <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Description</label>
                    <textarea name="description" x-model="editData.description" rows="2" class="w-full px-3.5 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white"></textarea>
                </div>
                ...
                <div class="pt-3 flex items-center justify-end gap-2">
                    ...
                    <button type="submit" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-md cursor-pointer">Update Category</button>
                </div>
            </form>
        </div>
    </div>
