    ...
    <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs">
        <form method="GET" action="{{ route('audit-logs.index') }}" class="flex flex-wrap items-center justify-between gap-3 text-xs">
            <div class="flex flex-wrap items-center gap-3">
                <select name="user_id" onchange="this.form.submit()" class="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-slate-200">
                ...
                <select name="module" onchange="this.form.submit()" class="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-slate-200">
                ...
                <select name="action" onchange="this.form.submit()" class="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-slate-200">
                ...
                <input type="date" name="date" value="{{ request('date') }}" onchange="this.form.submit()" class="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-slate-200">
            </div>

            @if(request()->hasAny(['user_id', 'module', 'action', 'date']))
            <a href="{{ route('audit-logs.index') }}" class="text-xs font-bold text-slate-600 hover:text-slate-900 dark:text-slate-400">
                Clear Filters ✕
            </a>
            @endif
        </form>
            ...
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition">
                        <td class="py-3 px-4 font-mono font-medium text-slate-800 dark:text-slate-200 whitespace-nowrap">{{ $log->created_at->format('d M Y, H:i:s') }}</td>
                        <td class="py-3 px-4">
                            @if($log->user)
                                <span class="font-bold text-slate-900 dark:text-white">{{ $log->user->name }}</span>
                                <span class="text-[11px] text-indigo-600 dark:text-indigo-400 font-bold block">{{ $log->user->role }}</span>
                            @else
                                <span class="text-slate-500 font-medium italic">System</span>
                            @endif
                        </td>
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 rounded-md font-bold text-[11px] bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200">
                                {{ $log->module }}
                            </span>
                        </td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4 text-slate-800 dark:text-slate-200 font-medium max-w-sm">{{ $log->description }}</td>
                        <td class="py-3 px-4 font-mono text-slate-600 dark:text-slate-400 font-semibold text-xs">{{ $log->ip_address ?? '-' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="py-10 text-center text-slate-500 font-medium">No audit logs found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
