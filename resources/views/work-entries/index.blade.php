...
<div class="space-y-6">
    ...
    <div class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
        <form method="GET" action="{{ route('work-entries.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs">
            <div>
                <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Date</label>
            ...
            <div>
                <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Employee</label>
            ...
            <div>
                <label class="block font-bold text-slate-700 dark:text-slate-300 uppercase mb-1 text-[11px] tracking-wider">Category</label>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('work-entries.index') }}" class="py-2.5 px-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 font-semibold rounded-xl text-center hover:bg-slate-200 transition">
                    Reset
                </a>
            </div>
        </form>
        ...
        <div class="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div class="text-slate-700 dark:text-slate-300 font-medium">
                Found <strong class="text-slate-900 dark:text-white">{{ $entries->total() }}</strong> entries in this view
            </div>
            <div class="flex items-center gap-3">
                ...
                <span class="text-slate-700 dark:text-slate-300 font-medium">
                    Active logged: <strong class="text-slate-900 dark:text-white">{{ $workedEmployeesCount }}/{{ $activeEmployeesCount }}</strong>
                </span>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 uppercase tracking-wider text-[11px] font-bold">
                        ...
                        <th class="py-3.5 px-4 font-extrabold">Remarks / Deliverables</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    ...
                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition">
                        <td class="py-3 px-4 font-mono text-slate-600 dark:text-slate-400 font-bold text-xs">#{{ $entry->id }}</td>
                        <td class="py-3 px-4 font-semibold text-slate-800 dark:text-slate-200 whitespace-nowrap">{{ $entry->date->format('d M, Y') }}</td>
                        <td class="py-3 px-4">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <div class="text-[11px] text-slate-600 dark:text-slate-400 font-medium">{{ $entry->employee->employee_code }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="py-3 px-4 text-slate-700 dark:text-slate-300 font-medium whitespace-nowrap">{{ $entry->employee->department->name ?? 'N/A' }}</td>
                        <td class="py-3 px-4">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold" style="background-color: {{ $entry->category->color }}20; color: {{ $entry->category->color }}">
                        ...
                        <td class="py-3 px-4 text-slate-800 dark:text-slate-200 font-medium max-w-sm">{{ $entry->remarks ?? '-' }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="py-10 text-center text-slate-600 font-medium">No work entries matched your filter.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
