...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    @if($adminEmployee && !auth()->user()->isSuperAdmin())
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="p-5 ui-panel">
            ...
            <div class="space-y-2.5">
                ...
                <div x-data="{ openSubtasks: false }" class="p-3 rounded-2xl bg-slate-50/80 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-800 space-y-2 group transition hover:border-[#0071e3]/40">
                    <div class="flex items-center justify-between gap-3">
                        <div class="flex items-center gap-2.5 min-w-0 flex-1">
                            <form method="POST" action="{{ route('todos.toggle', $todo) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit" title="Mark done" class="w-4.5 h-4.5 rounded-md border border-slate-300 dark:border-slate-600 hover:border-[#0071e3] bg-white dark:bg-slate-800 text-transparent hover:text-slate-300 flex items-center justify-center transition cursor-pointer">
                    ...
                    <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1 overflow-hidden">
                        <div class="bg-emerald-500 h-1 rounded-full transition-all duration-300" style="width: {{ round(($todo->completedSubtasksCount() / $todo->totalSubtasksCount()) * 100) }}%"></div>
                    </div>
                    ...
                    <div x-show="openSubtasks" x-cloak class="pt-1.5 space-y-1 border-t border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <form method="POST" action="{{ route('todos.toggleSubtask', $todo) }}" class="flex items-center gap-2 p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700/50 transition text-xs">
                            ...
                            <button type="submit" class="w-3.5 h-3.5 rounded border flex items-center justify-center transition cursor-pointer {{ !empty($subtask['completed']) ? 'bg-emerald-600 border-emerald-600 text-white' : 'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800' }}">
